    ...
    remote function onMessage(slack:MessageEvent event) returns error? {
        ...
        if (event.event?.channel.toString() == "C01TCFVPGGP") {
            if (!maps:hasKey(event.event, "parent_user_id")) {
                ...
                json getEmailResponse = check httpEndpoint->get(event.event?.user.toString(), headers = {"Authorization": slackBotToken}, targetType = json);

                // Get the slack message link from the timestamp
                string permLinkQueryParam = "channel=" + event.event?.channel.toString() + "&message_ts=" + event.event?.ts.toString();
                http:Client httpEndpointLink = check new ("https://slack.com/api/chat.getPermalink?");
                json getLinkResponse = check httpEndpointLink->get(permLinkQueryParam, headers = {"Authorization": slackBotToken}, targetType = json);
                ...
                check sheetsEndpoint->appendRowToSheet("16M0oNpMZSRG9yhJrfHPbha7m5-BMVdSuFhf5Yn9SvrA", "data", spreadSheetRowData, valueInputOption = "USER_ENTERED");
